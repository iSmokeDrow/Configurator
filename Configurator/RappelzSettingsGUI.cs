...
namespace Configurator
{
    ...
    public partial class RappelzSettingsGUI : Form
    {
        ...
        public List<ClientSetting> Settings;

        public RappelzSettingsGUI()
        {
            InitializeComponent();

            RappelzSettings.ReadOPT_v1();
            Settings = RappelzSettings.Settings;

            ChangeAvailability();

            setValues();
            gfxPreset_ValueChanged(null, EventArgs.Empty);
        }

        protected void ChangeAvailability()
        {
            titles.Enabled = RappelzSettings.Exists("PLAY_TITLE_SHOW");
            targetOutline.Enabled = RappelzSettings.Exists("PLAY_SELECT_OUTLINE_SHOW");
        }
        ...
        void lockOptions()
        {
            if (gfxPreset.Value > 0)
            {
                ...
                cameraCollisions.Enabled = false;
